...
(function(root, func) {
    ...
    } else if (typeof define === "function" && define.amd) define(func);
    ...
    if (window.PDFNet &&
        !PDFTron.skipPDFNetWebViewerWarning) console.warn("PDFNet.js and WebViewer.js have been included in the same context. See pdftron.com/kb_same_context for an explanation of why this could be an error in your application.");
    PDFTron.WebViewer = function WebViewer(options, element) {
        ...
        if (options.ui === "legacy") {
            ...
            if (options.mobileRedirect === undefined) options.mobileRedirect =
                true
        }
        if (options.fullAPI && options.pdftronServer) options.forceClientSideInit = true;
        this.options = Utils.extend({}, PDFTron.WebViewer.Options, options);
        var lastCharIndex = this.options.path.length - 1;
        if (lastCharIndex > 0 && this.options.path[lastCharIndex] !== "/") this.options.path += "/";
        ...
        if (!element) console.error("ViewerElement is not defined. This may be caused by calling the WebViewer constructor before the DOM is loaded, or an invalid selector. Please see https://www.pdftron.com/documentation/web/guides/quick-start for an example.");
        this.element = element;
        this.element.style.overflow = "hidden";
        if (!window.WebViewer) window.WebViewer = {};
        ...
        window.addEventListener("message", function(event) {
            if (event.source !== me.iframe.contentWindow) return;
            if (event.data === "requestl") event.source.postMessage({
                type: "responsel",
        ...
        if (this.options.autoCreate) this.create()
    };
    PDFTron.WebViewer.prototype = {
        ...
        _create: function() {
            var me = this;
            me.rcId = ((1 + Math.random()) * 65536 |
                0).toString(16).substring(1);
            if (typeof this._trigger === "undefined") this._trigger = function(type, data) {
        ...
        _validateOptions: function(options) {
            for (var option in options) {
                ...
                if (!(option in allOptions)) console.warn(option + " is not a valid option name. See https://www.pdftron.com/api/web/PDFTron.WebViewer.html#Options__anchor for all available options.")
            }
            if (options.enableRedaction &&
                !(options.fullAPI || options.pdftronServer)) console.warn("FullAPI or WebViewer Server is needed to apply redactions")
        },
        ...
        _createViewer: function(viewers) {
            ...
            if (this.isMobileDevice()) {
                if (this.options.documentType &&
                    this.options.documentType !== "xod" && !this._testWebAssembly()) {
            ...
            if (me.selectedType === "html5") me._createHTML5();
            else if (me.selectedType === "html5Mobile") me._createHTML5Mobile();
            else {
                var supportError;
                if (corsError) supportError = "This browser does not support cross origin requests. Please configure xdomain to support CORS.";
                else if (allowHTML5) supportError = "Please use an HTML5 compatible browser.";
                if (supportError) {
            ...
            try {
                ...
                if (typeof me.options.encryption !== "undefined") {
                    ...
                    viewerWindow.ControlUtils.byteRangeCheck(function(status) {
                        if (status === 200) loadOptions.streaming = true;
                ...
                if (me.instance.docViewer.getDocument() === null) viewerWindow.$(iframe.contentDocument).on("documentLoaded", function(event) {
                    me._trigger(event.type)
                });
                else me._trigger("documentLoaded");
                viewerWindow.$(iframe.contentDocument).on("displayModeChanged layoutModeChanged zoomChanged pageChanged fitModeChanged toolModeChanged printProgressChanged error", function() {
        ...
        _getHTML5OptionsURL: function() {
            if (this.selectedType === "html5") {
                if (this.options.html5Options) Utils.extend(this.options,
                    this.options.html5Options)
            } else if (this.selectedType === "html5Mobile")
                if (this.options.html5MobileOptions) Utils.extend(this.options, this.options.html5MobileOptions);
            var options = this.options;
            var url = "";

            function acceptBackendUrl(officeEnabled, pdfEnabled) {
                if (officeEnabled) url += options.backendType ? "&office=" + options.backendType : "&office=auto";
                if (pdfEnabled) url += options.backendType ? "&pdf=" + options.backendType : "&pdf=auto"
            }
            if (options.initialDoc) url += "#d=" + options.initialDoc;
            if (typeof options.backendType ===
                "undefined") options.backendType = options.pdfBackend;
            if (options.ui === "legacy") {
                var officeEnabled = false,
                    pdfEnabled = false;
                var periodPosition;
                ...
                if (options.documentType === "pdf" || options.documentType === "all" || typeof options.documentType === "undefined" && this._isPDFExtension(extension)) pdfEnabled = true;
                if (options.documentType === "office" || options.documentType === "all" ||
                    typeof options.documentType === "undefined" && this._isOfficeExtension(extension)) officeEnabled = true;
                if (options.documentType) url += "&documentType=" + options.documentType;
                acceptBackendUrl(officeEnabled, pdfEnabled)
            } else {
                var _extension;
                var _preloadWorker;
                if (options.documentType && options.documentType === "xod") _extension = options.documentType;
                if (options.preloadWorker && options.preloadWorker === "xod") _extension = options.preloadWorker;
                if (options.extension) _extension = options.extension;
                if (_extension) url += "&extension=" +
                    _extension;
                if (options.documentType && options.documentType !== "xod") _preloadWorker = options.documentType;
                if (options.preloadWorker && options.preloadWorker !== "xod") _preloadWorker = options.preloadWorker;
                if (_preloadWorker) url += "&preloadWorker=" + _preloadWorker;
                if (options.backendType) url += "&pdf=" + options.backendType + "&office=" + options.backendType
            }
            if (options.streaming) url += "&streaming=" + options.streaming;
            if (options.externalPath) {
                var path = this._correctRelativePath(options.externalPath);
                path = encodeURIComponent(path);
                url += "&p=" + path
            }
            if (options.encryption) url += "&auto_load=false";
            if (options.enableAnnotations) url += "&a=1";
            ...
            if (options.serverUrlHeaders) url += "&serverUrlHeaders=" + JSON.stringify(options.serverUrlHeaders);
            if (options.documentId) url += "&did=" + options.documentId;
            if (options.css) {
            ...
            if (options.disableI18n && options.ui !== "legacy") url += "&disableI18n=1";
            if (options.enableOfflineMode) url += "&offline=1";
            if (options.startOffline) url += "&startOffline=1";
            if (options.enableReadOnlyMode ||
                options.isReadOnly) url += "&readonly=1";
            if (options.hideAnnotationPanel) url += "&hideAnnotationPanel=1";
            if (typeof options.annotationUser !== "undefined") url += "&user=" + options.annotationUser;
            if (typeof options.annotationAdmin !== "undefined" || typeof options.isAdminUser !== "undefined") url += "&admin=" + (options.annotationAdmin || options.isAdminUser ? 1 : 0);
            if (typeof options.custom !== "undefined") url += "&custom=" + encodeURIComponent(options.custom);
            if (typeof options.showLocalFilePicker !== "undefined" || typeof options.enableFilePicker !==
                "undefined") url += "&filepicker=" + (options.showLocalFilePicker || options.enableFilePicker ? 1 : 0);
            if (options.ui === "legacy" && typeof options.preloadPDFWorker !== "undefined") url += "&preloadWorker=" + (options.preloadPDFWorker ? 1 : 0);
            if (typeof options.fullAPI !== "undefined") url += "&pdfnet=" + (options.fullAPI ? 1 : 0);
            if (typeof options.enableRedaction !== "undefined") url += "&enableRedaction=" + (options.enableRedaction ? 1 : 0);
            if (typeof options.enableMeasurement !== "undefined") url += "&enableMeasurement=" + (options.enableMeasurement ?
                1 : 0);
            if (typeof options.showToolbarControl !== "undefined") url += "&toolbar=" + (options.showToolbarControl ? "true" : "false");
            if (typeof options.showPageHistoryButtons !== "undefined") url += "&pageHistory=" + (options.showPageHistoryButtons ? 1 : 0);
            if (typeof options.xdomainProxyUrl !== "undefined") {
                var urls;
                if (typeof options.xdomainProxyUrl === "string") urls = {
                    url: options.xdomainProxyUrl
                };
                else urls = options.xdomainProxyUrl;
                url += "&xdomain_urls=" + encodeURIComponent(JSON.stringify(urls))
            }
            if (options.azureWorkaround || options.enableAzureWorkaround) url +=
                "&azureWorkaround=1";
            if (!options.useDownloader) url += "&useDownloader=0";
            if (options.disableWebsockets) url += "&disableWebsockets=1";
            if (options.subzero) url += "&subzero=1";
            if (options.forceClientSideInit) url += "&forceClientSideInit=1";
            if (typeof options.workerTransportPromise !== "undefined") url += "&useSharedWorker=" + (options.workerTransportPromise ? "true" : "false");
            if (typeof options.pdftronServer !== "undefined" && options.pdftronServer) url += "&pdftronServer=" + encodeURIComponent(options.pdftronServer);
            if (url.length >
                0 && url[0] === "&") url = "#" + url.slice(1);
            return url
        },
        _createHTML5: function() {
            ...
            this.iframe = rcFrame;
            if (this.options.backgroundColor) rcFrame.setAttribute("data-bgcolor",
                this.options.backgroundColor);
            if (this.options.assetPath) rcFrame.setAttribute("data-assetpath", encodeURIComponent(this.options.assetPath));
            rcFrame.addEventListener("load", function() {
                try {
                    me.instance = this.contentWindow.readerControl;
                    var iframe = this;
                    if (typeof me.instance === "undefined") this.contentWindow.$(this.contentDocument).on("viewerLoaded", function() {
                        me.instance = iframe.contentWindow.readerControl;
                        me._viewerLoaded(iframe)
                    });
                    else me._viewerLoaded(iframe)
                } catch (error) {
                    me._viewerLoaded(iframe)
                }
            });
        ...
        _createHTML5Mobile: function() {
            ...
            rcFrame.addEventListener("load", function() {
                try {
                    me.instance = this.contentWindow.readerControl;
                    var iframe = this;
                    if (typeof me.instance === "undefined") this.contentWindow.$(this.contentDocument).bind("viewerLoaded", function() {
                        me.instance = iframe.contentWindow.readerControl;
                        me._viewerLoaded(iframe)
                    });
                    else me._viewerLoaded(iframe)
        ...
        getInstance: function() {
            if (!this.instance) console.error("The viewer instance is not defined yet. Try calling getInstance() in the ready event listener, or check https://www.pdftron.com/documentation/web/guides/ui/apis for a detailed example.");
        ...
        getLayoutMode: function() {
            ...
            if (layoutMode === displayModes.Single) return PDFTron.WebViewer.LayoutMode.Single;
            else if (layoutMode === displayModes.Continuous) return PDFTron.WebViewer.LayoutMode.Continuous;
            else if (layoutMode === displayModes.Facing) return PDFTron.WebViewer.LayoutMode.Facing;
            else if (layoutMode === displayModes.FacingContinuous) return PDFTron.WebViewer.LayoutMode.FacingContinuous;
            else if (layoutMode === displayModes.Cover) return PDFTron.WebViewer.LayoutMode.FacingCoverContinuous;
            else if (layoutMode === displayModes.CoverFacing) return PDFTron.WebViewer.LayoutMode.FacingCover;
            return undefined
        },
        setLayoutMode: function(layoutMode) {
            var displayModes = CoreControls.DisplayModes;
            var displayMode = displayModes.Continuous;
            if (layoutMode === PDFTron.WebViewer.LayoutMode.Single) displayMode = displayModes.Single;
            else if (layoutMode === PDFTron.WebViewer.LayoutMode.Continuous) displayMode = displayModes.Continuous;
            else if (layoutMode === PDFTron.WebViewer.LayoutMode.Facing) displayMode = displayModes.Facing;
            else if (layoutMode ===
                PDFTron.WebViewer.LayoutMode.FacingContinuous) displayMode = displayModes.FacingContinuous;
            else if (layoutMode === PDFTron.WebViewer.LayoutMode.FacingCover) displayMode = displayModes.CoverFacing;
            else if (layoutMode === PDFTron.WebViewer.LayoutMode.FacingCoverContinuous) displayMode = displayModes.Cover;
        ...
        setFitMode: function(fitMode) {
            if (fitMode === PDFTron.WebViewer.FitMode.FitWidth) this.fitWidth();
            else if (fitMode === PDFTron.WebViewer.FitMode.FitHeight) this.fitHeight();
            else if (fitMode === PDFTron.WebViewer.FitMode.FitPage) this.fitPage();
            else if (fitMode === PDFTron.WebViewer.FitMode.Zoom) this.zoom();
            else console.warn("Unsupported fit mode: " + fitMode)
        },
        zoom: function() {
            var docViewer = this.getInstance().docViewer;
        ...
        loadDocument: function(url, options) {
            ...
            if (typeof loadOptions.documentId !== "undefined") this.getInstance().docId =
        ...
        searchText: function(pattern, searchModes) {
            ...
            if (typeof modes === "string") modes = searchModes.split(",");
            if (typeof modes !== "undefined")
                for (var i = 0; i < modes.length; i++)
                    if (modes[i] === "CaseSensitive") mode += 1;
                    else if (modes[i] === "WholeWord") mode += 2;
            else if (modes[i] === "SearchUp") mode += 4;
            else if (modes[i] === "PageStop") mode += 8;
            else if (modes[i] === "ProvideQuads") mode += 16;
            else if (modes[i] ===
                "AmbientString") mode += 32;
            if (typeof searchModes === "undefined") this.getInstance().searchText(pattern);
            else this.getInstance().searchText(pattern, mode)
        },
        ...
        startPrintJob: function(pages) {
            if (this.selectedType === "html5") this.getInstance().startPrintJob(pages);
            else console.warn("Unsupported method startPrintJob")
        },
        endPrintJob: function() {
            if (this.selectedType === "html5") this.getInstance().endPrintJob();
            else console.warn("Unsupported method endPrintJob")
        },
        ...
        _supports: function(type) {
            if (type === this.selectedType) return true;
            for (var i = 1; i < arguments.length; i++)
                if (arguments[i] === this.selectedType) return true;
            return false
        },
        _testCORS: function() {
        ...
        isMobileDevice: function() {
            return !this.isIE() && (this.scrollbarWidth() === 0 && navigator.userAgent.match(/Edge/i) || navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/webOS/i) || navigator.userAgent.match(/iPhone/i) ||
        ...
        runInIframe: function(func) {
            ...
            if (this.getInstance()) setTimeout(callFunc, 0);
            else this.element.addEventListener("ready", callFunc)
        }
    };
    ...
    PDFTron.WebViewer.User = function(username, isAdmin, isReadOnly) {
        ...
        if (typeof isAdmin !== "undefined") this.isAdmin = isAdmin;
        else this.isAdmin = false;
        if (typeof isReadOnly !== "undefined") this.isReadOnly = isReadOnly;
